...
class Game {
  ...
  getPlayer(jid) {
    ...
    // Level scaling
    u.game.atk = 10 + Math.floor(u.level * 1.5);
    u.game.def = 5 + Math.floor(u.level * 0.8);
  ...
  battleAction(jid, action) {
    ...
    if (b.monsterHp <= 0) {
      ...
      delete battles[jid];
      // Update player HP
      player.hp = Math.max(0, b.playerHp);
      db._save();
  ...
  fleeBattle(jid) {
    ...
    player.hp = b.playerHp;
    db._save();
    if (fled) return { msg: `🏃 *KABUR!*\nKamu berhasil melarikan diri dari ${b.monster.emoji} *${b.monster.name}*!` };
    const penalty = this._rand(10, 30);
    b.playerHp = Math.max(0, b.playerHp - penalty);
    player.hp = b.playerHp;
    db._save();
    return { msg: `🚫 *GAGAL KABUR!*\n${b.monster.emoji} *${b.monster.name}* menangkapmu! -${penalty} HP` };
  }
