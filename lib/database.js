...
class Database {
  ...
  getUser(jid) {
    ...
    if (u) this._checkPremiumExpiry(u);
    return u || null;
  }
  ...
  _checkPremiumExpiry(u) {
    if (u.premium && u.premiumExpiry > 0 && u.premiumExpiry <= Date.now()) {
      u.premium = false;
      u.premiumExpiry = 0;
      u.role = "user";
  ...
  isPremium(jid) {
    ...
    return u.premium && u.premiumExpiry > Date.now();
  }
...
╚═══════════════════════╝`;
  ...
  getStats() {
    ...
    const topLevel = users.reduce((max, u) => u.level > max ? u.level : max, 0);
    return { total, premium, admins, owners, totalXp, totalMoney, topLevel };
  }
}
